...
predict(m0)
mse(annual_sales$total_sales,predict(m0))
mean((obs_train_m0-pred_train_m0)^2)
...
tm0_graph <- tm0 %>% ggplot() + geom_point(aes(x=x,y=obs_train_m0,color='Observations')) +
  ...
  ylab('Sales')+xlab('Year') +
  ggtitle("m0: Total annual sales trend analysis and forecast for Hass avocado in the US") +
  scale_y_continuous(labels = scales::comma) + 
  theme_minimal() +
  theme(legend.position="right", legend.box = "vertical") +
  scale_color_discrete(name = "Data")
...
price_vs_sales_train <- train %>%
  ...
  select(c("total_sales","mean_price","year"),p.value) %>%
  ungroup()
...
price_vs_sales_test <- test %>%
  ...
  summarize(total_sales = sum(total_volume), mean_price = mean(average_price))%>%
  mutate(broom::glance(lm(total_sales ~ mean_price))) %>%
  select(c("total_sales","mean_price","year"),adj.r.squared,p.value) %>%
  ungroup()
...
tm1_graph <- tm1 %>% ggplot() + geom_point(aes(x=x,y=obs_train_m1,color='Observations')) + 
  ...
  ylab('Sales')+xlab('Average daily Price') +
  ggtitle("m1: Daily sales trend analysis in accordance to price ") +
  scale_y_continuous(labels = scales::comma) + 
  theme_minimal() +
  theme(legend.position="right", legend.box = "vertical") +
  scale_color_discrete(name = "Data")
...
#subset total daily sales on different train, validate and test set
  ...
  tm2_graph <- tm2 %>% ggplot() + geom_point(aes(x=x,y=obs_train_m2,color='Observations')) + 
    geom_line(aes(x=x,y=pred_train_m2,color='Predictions')) + 
    ylab('Sales')+xlab('Dates') +
    ggtitle("m2: Daily sales trend analysis and forecast for Hass avocado in the US") +
    scale_y_continuous(labels = scales::comma) + 
    theme_minimal() +
    theme(legend.position="right",legend.box = "vertical") +
    scale_color_discrete(name = "Data")
...
plot(annual_sales_train$residuals)
